<header id="masthead" class="site-header header-four">
	<div class="top-header">
		...
		<?php if( !get_theme_mod( 'disable_mobile_top_header', false ) ){ ?>
			<?php if( has_nav_menu( 'menu-3') || ( !get_theme_mod( 'disable_header_social_links', false ) && bosa_has_social() ) || is_active_sidebar( 'menu-sidebar' ) || get_theme_mod( 'header_advertisement_banner', '' ) != '' || !get_theme_mod( 'disable_search_icon', false ) ){ ?>
				...
				<div class="col-md-3 d-none d-md-block">
					<?php if ( class_exists('WooCommerce' ) ) { ?>
					    <div class="header-right hidden-xs" >
					        <?php
					        if( !get_theme_mod( 'disable_woocommerce_compare', false ) ){bosa_shop_head_compare();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_wishlist', false ) ){bosa_shop_head_wishlist();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_account', false ) ){bosa_shop_my_account();
					        }
					        if( !get_theme_mod( 'disable_woocommerce_cart', false ) ){bosa_shop_header_cart();
					        }
					        ?>
					    </div>	
					<?php } ?>
			...
			<div class="row">
				<div class="col-lg-7 d-none d-lg-block">
					<nav id="site-navigation" class="main-navigation d-none d-lg-flex">
						<button class="menu-toggle" aria-controls="primary-menu" aria-expanded="false"><?php esc_html_e( 'Primary Menu', 'bosa-shop' ); ?></button>
					...
					<?php if ( class_exists('WooCommerce' ) ) {
				    	if( !get_theme_mod( 'disable_search_icon', false ) ){ ?>
		    	            <form class="header-search-form" role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>">
		    	                ...
		    	                <input class="header-search-input" name="s" type="text" placeholder="<?php esc_attr_e('Search products...', 'bosa-shop'); ?>"/>
		    	                <div class="d-inline-block"> 
		    	                	<select class="header-search-select" name="product_cat">
			    	                    <option value=""><?php esc_html_e('All Categories', 'bosa-shop'); ?></option> 
			    	                    <?php
